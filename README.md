...
- **🛡️ Strict Pre-Execution Feasibility Engine (`Restaurant.IsActionDoable`)** — Zero-assumption pre-flight validation of avatar hand state, station availability, table cleanliness, active orders, and affordability before initiating any movement or interaction, eliminating wasted teleports and rejected actions.
- **🏰 Quest-Gated Land & Floor Expansions (`AutoQuestGateExpansions`)** — Land and multi-story floor unlocks are strictly manual opt-in and quest-guarded. In-world pad inspection, attribute checks, and reactive toast interception (`"need to complete quest first"`) immediately pause land acquisition and prioritize quest fulfillment until unlocked.
...
- **📋 "What I Can Buy" Interactive Catalog Inspector** — Real-time shop and plot scanner that breaks down available upgrades into categorized types (🏰 Land & Floors, 🍳 Cooking Appliances, 🪑 Dining Furniture, 🍽️ Kitchen Equipment, 🌿 Decor, 👨‍🍳 Staff) with live affordability badges (`[ AFFORDABLE ]`, `[ NEED +$X ]`, `[ 🔒 QUEST LOCKED ]`) and direct buy buttons.
